...
def _initial_scene_state() -> dict[str, Any]:
    return {
        ...
        "edit": {
            # Slideshow edit state — independent from video.
            # zones_json + thumb persist even after video is regenerated by provider.
            "status": "pending",
            "zones_json": None,
            "thumb_path": None,
            "last_render_at": None,
...
class Project:
    ...
    def _reconcile(state: dict[str, Any], scenes_json: ScenesJson) -> dict[str, Any]:
        ...
        for existing scenes so older state.json files migrate cleanly.

        Recovery: any asset stuck on status='generating' is reset to 'failed'.
        This can only mean the previous session crashed mid-render — no worker
        is actually running once we reach load().
        """
        scenes = state.setdefault("scenes", {})
        current_ids = {s.id for s in scenes_json.scenes}
        stuck_keys = ("image", "video", "edit")
        stuck_count = 0
        for scene in scenes_json.scenes:
            if scene.id not in scenes:
                ...
                # Migrate: ensure all keys from initial state exist (forward-compat)
                template = _initial_scene_state()
                for k, v in template.items():
                    if k not in scenes[scene.id]:
                        scenes[scene.id][k] = v
                # Reset stuck 'generating' from crashed previous session
                for asset_key in stuck_keys:
                    asset_state = scenes[scene.id].get(asset_key)
                    if isinstance(asset_state, dict) and asset_state.get("status") == "generating":
                        asset_state["status"] = "failed"
                        asset_state["fail_reason"] = "stuck from previous session (crashed before completion)"
                        stuck_count += 1
                        log.warning(
                            f"Recovered stuck state: {scene.id}.{asset_key} "
                            f"'generating' → 'failed'"
                        )
        if stuck_count > 0:
            log.info(f"Reset {stuck_count} stuck 'generating' asset(s) from previous session")
