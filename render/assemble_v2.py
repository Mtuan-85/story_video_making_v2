"""Phase 5 assemble: concat scene clips + apply ASS subtitle (2-pass).

Coexists with Sprint 2 `render.assemble`. Phase 6 will switch the UI.
"""
...
def assemble_concat(scene_paths: list[Path], output_path: Path) -> Path:
    ...
    Requires all clips share codec/timebase/resolution — composite_v2 enforces
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="assemble_v2_"))
    ...
    if result.returncode != 0:
        log.error(f"assemble_v2 failed: {(result.stderr or '')[-1500:]}")
        raise RuntimeError("FFmpeg assemble_v2 failed")
