"""Phase 5 composite: visual + voice slice (NO subtitle, applied later).

Coexists with the Sprint 2 `render.composite` module — Phase 6 will switch
the UI to this v2 path and the legacy module can be retired.
"""
...
def composite_scene_v2(
    ...
    log.info(
        f"composite_v2 {scene['id']}: visual={visual_type} effect={effect} "
    ...
    if result.returncode != 0:
        log.error(f"composite_v2 {scene['id']} failed: {(result.stderr or '')[-1500:]}")
        raise RuntimeError(f"FFmpeg composite_v2 failed for {scene['id']}")
