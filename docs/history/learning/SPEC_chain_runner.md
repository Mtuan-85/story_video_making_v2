...
│                                                              │
│  1. Load state.json                                         │
│  2. Validate prerequisite (input file của step có chưa)    │
│  3. async_playwright() + launch_brave_over_cdp             │
│  4. Run step (SPEC_cdp_chatgpt_flow.md flow)               │
│  5. Save project_Sn.json + project_Sn.raw.txt              │
│  6. Update state.json atomic                                │
│  7. Print log markers stable cho GUI parse                  │
│  8. sys.exit(0/1/2)                                         │
└────────────────────────────────────────────────────────────┘
```
...
│   └── state.py                ★ MỚI — state I/O + prerequisite check
├── instruction/
│   └── link_custom_gpt.md
...
`running` được set khi worker bắt đầu, để GUI biết step nào đang chạy
nếu nhiều worker quay vòng. Khi worker exit (clean hoặc không), GUI
phải treat `running` là indeterminate — kiểm tra QProcess state để
quyết định.

Nếu `state.json` corrupt khi load → backup thành `state.json.corrupt.bak`
và tạo mới. Output files (`project_S*.json`) không bị đụng.
...
|---|---|---|
| 0 | Success, output ghi xong | Mark step done, có thể chạy step tiếp |
| 1 | Step fail (CDP error, ChatGPT error, parse fail, etc.) | Show error trong log, suggest retry / kill brave |
| 2 | Prerequisite missing (input file chưa có) | Nhắc user chạy step trước đó |
| 3 | (reserved) Stop signal received gracefully | Show "Stopped" |
...
GUI **không** parse log chi tiết bên trong. Chỉ parse 4 marker trên.
Log chi tiết hiển thị raw cho user đọc.
...
- Worker **không poll state file** trong khi chạy. Chỉ load 1 lần đầu
  + 1 lần reload trước khi save final state.
...
### Case 1: Worker exit 1 (step fail)
...
### Case 3: Hang permanent (worker stuck)
...
### Case 4: CDP stale (connect_over_cdp hang)
...
- ❌ Multi-worker concurrent (chạy nhiều step song song)
...
| Rerun S3 sau S1+S2 done | Chạy lại từ S1 | Click Run S3, dùng output S2 đã có |
